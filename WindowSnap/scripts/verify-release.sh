...
verify_app() {
  ...
  /usr/bin/lipo "$executable" -verify_arch arm64 || die "$label lacks arm64 ($architectures)"
  /usr/bin/lipo "$executable" -verify_arch x86_64 || die "$label lacks x86_64 ($architectures)"
  /usr/bin/codesign --verify --strict --verbose=2 "$candidate"
  /usr/bin/codesign -d --verbose=4 "$candidate" 2>&1 | grep 'Runtime Version' >/dev/null || die "$label lacks Hardened Runtime"
  /usr/bin/codesign -d --verbose=4 "$candidate" 2>&1 | grep 'Timestamp=' >/dev/null || die "$label lacks a secure timestamp"
